...
export const CodeItem: React.FC<CodeItemProps> = ({ otp, setOtp }) => {
    ...
    function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>, index: number) {
        ...
        if (arrowKeys.includes(e.key)) {
            ...
            const nextInput = document.querySelector(`input[type="text"][data-index="${newIndex}"]`) as HTMLInputElement;
            if (nextInput) {
                nextInput.focus();
            }
            ...
            if (index > 0) {
                ...
                const prevInput = document.querySelector(`input[type="text"][data-index="${prevIndex}"]`) as HTMLInputElement;
                if (prevInput) {
                    prevInput.focus();
                }
            }
