...
export const PreferencesForm: React.FC<{ }> = ({  }) => {
    ...
    const labels = ["Music", "Sport", "Game", "Cooking", "Art", "Technology",
        "Health & Fitness", "Fashion", "Travel"];
    ...
    return (
        <div className={styles.div}>
                    ...
                    <div>
                        <div>
                            {labels.map((label, index) => (
                                <span key={index} onClick={() => handleSpanClick(label)}
                                      className={`${selectedLabels.includes(label) ? styles.selected : ''}`}>{label}</span>
                            ))}
                        </div>
                    </div>
                </div>
            ...
            <footer>
                <button onClick={() => navigate('/auth/login')}>{t('button_finish')}</button>
            </footer>
        </div>
    );
}