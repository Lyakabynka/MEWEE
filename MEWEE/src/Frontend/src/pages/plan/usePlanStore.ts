...
interface IPlanStore {
  ...
  createScheduledPlan: (
    requestModel: ICreateScheduledPlanRequest
  ) => Promise<void>;
...
export const usePlanStore = create<IPlanStore>()((set, get) => ({
  ...
  createScheduledPlan: async (requestModel: ICreateScheduledPlanRequest) => {
    ...
    const response = await $api.post<any>(
      ENDPOINTS.SCHEDULED_PLAN.CREATE_SCHEDULED_PLAN,
      requestModel
    );
  ...
  deleteScheduledPlan: async (id: string) => {
    ...
    const response = await $api.delete<any>(
      ENDPOINTS.SCHEDULED_PLAN.DELETE_SCHEDULED_PLAN.replace("{id:guid}", id)
    );
  ...
  getScheduledPlans: async (planId: string) => {
    ...
    const response = await $api.get<IScheduledPlan[]>(
