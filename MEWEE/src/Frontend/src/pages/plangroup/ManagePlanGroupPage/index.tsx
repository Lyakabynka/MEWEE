...
import {
  IPlan,
  IPlanPlanGroup,
  ISetPlanToPlanGroupRequestModel,
} from "../../../entities";
...
import { ManagePlanGroupSaveButtonPlaceHolder } from "../../../features/exportFeaturesComponents";
import "./index.css";
...
export const ManagePlanGroupPage = () => {
  ...
  const [planPlanGroupGridApi, setPlanPlanGroupGridApi] =
    useState<GridApi<IPlanPlanGroup>>();
  ...
  useEffect(() => {
    $api
      .get(ENDPOINTS.PLANGROUP.GET_PLANGROUP.replace(`{id:guid}`, String(id)))
      .then((response) => {
  ...
  const handlePlansRowDoubleClicked = (
    e: RowDoubleClickedEvent<IPlan, any>
  ) => {
    setPlanGroup({
      ...planGroup!,
      planPlanGroups: [
        ...planGroup!.planPlanGroups,
        {
          id: "",
          plan: {
            id: e.data!.id,
  ...
  const handlePlanPlanGroupsDragStopped = (
    e: DragStoppedEvent<IPlanPlanGroup, any>
  ) => {
    let planPlanGroups: IPlanPlanGroup[] = e.api
      .getRenderedNodes()
  ...
  const handlePlanPlanGroupsDoubleClicked = (
    e: RowDoubleClickedEvent<IPlanPlanGroup, any>
  ) => {
    setPlanGroup({
      ...planGroup!,
      planPlanGroups: planGroup!.planPlanGroups.filter((pg) => {
        console.log(pg.id);
  ...
  const handleClick = () => {
    const setPlanToPlanGroupRequestModels = planPlanGroupGridApi
      ?.getRenderedNodes()
    ...
    if (
      setPlanToPlanGroupRequestModels === undefined ||
  ...
  return (
    <>
      ...
      >
        <div
          className="ag-theme-alpine-dark custom-theme-group"
          style={{ width: "47.5%" }}
        >
          <Box
            sx={{
        ...
        <div
          className="ag-theme-alpine custom-theme-plans"
          style={{ width: "47.5%" }}
        >
          <Box
            sx={{
