...
export const ScheduledPlanPage = () => {
  ...
  const getScheduledPlans = () => {
    $api
      .get(
        ENDPOINTS.SCHEDULED_PLAN.GET_SCHEDULED_PLANS.replace(
          "{planId:guid}",
          String(id)
        )
      )
  ...
  const handleScheduledPlansDoubleClicked = (
    e: RowDoubleClickedEvent<IScheduledPlan>
  ) => {
    let id = e.data?.id;
    console.log(id);
    ...
    if (id !== undefined || id !== null) {
      ...
      $api
        .delete(
          ENDPOINTS.SCHEDULED_PLAN.DELETE_SCHEDULED_PLAN.replace(
            "{id:guid}",
            id
          )
        )
        .then((response) => {
          if (response.status === 200) {
            setPlanSchedule({
              ...planSchedule!,
              scheduledPlans: planSchedule!.scheduledPlans.filter(
                (sp) => sp.id !== id
              ),
            });
          }
