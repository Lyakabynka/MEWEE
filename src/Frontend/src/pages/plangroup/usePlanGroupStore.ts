...
interface IPlanGroupStore {
  ...
  setPlansToPlanGroup: (
    id: string,
    requestModel: ISetPlanToPlanGroupRequestModel[]
  ) => Promise<void>;

  deletePlanGroup: (id: string) => Promise<void>;

  createScheduledPlanGroup: (
    requetModel: ICreateScheduledPlanGroupRequest
  ) => Promise<void>;
}
...
export const usePlanGroupStore = create<IPlanGroupStore>()((set, get) => ({
  ...
  createPlanGroup: async (requestModel: ICreatePlanGroupRequest) => {
    ...
    const response = await $api.post<IPlanGroup>(
      ENDPOINTS.PLANGROUP.CREATE_PLANGROUP,
      requestModel
    );
  ...
  setPlansToPlanGroup: async (
    id: string,
    requestModels: ISetPlanToPlanGroupRequestModel[]
  ) => {
    set({ isLoading: true });

    const response = await $api.post(
      ENDPOINTS.PLANGROUP.SET_PLANS_TO_PLANGROUP.replace("{id:guid}", id),
      requestModels
    );
  ...
  deletePlanGroup: async (id: string) => {
    ...
    const response = await $api.delete(
      ENDPOINTS.PLANGROUP.DELETE_PLANGROUP.replace("{id:guid}", id)
    );
  ...
  createScheduledPlanGroup: async (
    requestModel: ICreateScheduledPlanGroupRequest
  ) => {
    ...
    const response = await $api.post<any>(
      ENDPOINTS.SCHEDULED_PLANGROUP.CREATE_SCHEDULED_PLANGROUP,
      requestModel
    );
